...
ESP_START_BYTE_0 = 0x22
ESP_START_BYTE_1 = 0x69
        ...
        while not self.stop.is_set():
            for socket, event in poller.poll(500):
                if socket == self.tx and event == zmq.POLLIN:
                    ...
                    log.debug("> %s", ''.join(str(hexlify(p)) for p in msg_parts))
...
class SerialTx(Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            #log.debug("Sending serial message %s", ''.join(hex(b) for b in msg))
            header = ESP_START_BYTE_0.to_bytes(1, "little") + ESP_START_BYTE_1.to_bytes(1, "little") + len(msg).to_bytes(1, "little")
            #log.debug("Sending serial header %s", ''.join(hex(b) for b in header))
            #log.debug("Sending serial header %s", ''.join(hex(b) for b in (header + msg)))
...
class SerialRx(Thread):
    ...
    def read_messages(self):
        while True:
            ...
            while b != ESP_START_BYTE_0.to_bytes(1, "little"):
                b = self.serial_port.read(1)
            log.debug("Waiting for start byte 2")
            while b == ESP_START_BYTE_0.to_bytes(1, "little"):
                b = self.serial_port.read(1)
            if b != ESP_START_BYTE_1.to_bytes(1, "little"):
                continue
